...
match(void)
{
    ...
    for (item = items; item && item->text; item++) {
        ...
        else if (!use_prefix) // Prefix Completion
    ...
    if (!use_prefix && lsubstr) { // Prefix Completion
        if (matches) {
...
keypress(XKeyEvent *ev)
{
    char buf[32];
    int len;
    struct item * item; // Prefix Completion
    KeySym ksym;
    Status status;
...
usage(void)
{
    fputs("usage: dmenu [-bfivxF] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
...
main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
        ...
        if (!strcmp(argv[i], "-v")) {      /* prints version information */
            ...
            fstrstr = cistrstr;
        /* Begin Prefix Completion */
        } else if (!strcmp(argv[i], "-x")) /* invert use prefix */
